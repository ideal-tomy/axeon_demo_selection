...
この文書は **10件の「カード → 紹介ページ → 外部デモ →（必要なものだけLP）→ 相談」をどう通すか** だけを持つ。`AXEON_pf` は触らない。
...
- 卸は `data.js` に `listed:false` で登録。カード化は公開URLと波0のあと。運送（`logistics-dispatch`）・DD（`dd-ma`）は登録済み・流れ完成。
- 卸は体験版のカード公開まで一覧に出さない。
...
| 波 | 対象 | 内容 | 状態 |
|---|---|---|---|
| 0 | 厳選版サイト | 掲載フィルタ、全件一覧の扱い、関連範囲、戻り先URLの決め方 | [x] 完了。[wave0-plan.md](impl/wave0-plan.md) |
...
1. 確定9件を1デモずつ：実画面の撮影＋紹介文の書き直し（建設から。卸は公開URL後）
2. （任意）卸の公開URLが付いたら `listed:true` と `featuredOrder:10`

波0（掲載9件・全件導線切断・戻り先文書化）は 2026-09-22 完了。波1〜3の流れ完成は既存どおり。
